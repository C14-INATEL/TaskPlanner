...
5.  Configure as variáveis de ambiente necessárias. Crie um arquivo `.env` na pasta [backend/](file:///C:/Users/lucas/Documents/INATEL/P8/C14/projeto/TaskPlanner/backend) contendo:
    ...
    ```
    *(Ajuste o usuário, senha e porta do banco de acordo com a sua configuração local. Por padrão, se não for especificado, o backend usará o PostgreSQL rodando localmente no endereço acima)*
...
#### [Nome do Integrante 4] (DevOps & Documentação)

| # | Prompt Real Utilizado | Status | Justificativa Técnica |
|---|---------------------|--------|-----------------------|
| 1 | "Como escrever um arquivo `.gitignore` padrão para um repositório monorepo contendo projetos Flask e Next.js?" | **Aceita** | A lista gerada cobriu eficientemente todas as dependências locais e caches de compilação, sendo salva diretamente no repositório. |
...
*   **`[Nome do Integrante 4]` (DevOps & Documentação)**: Focado no gerenciamento de versionamento do repositório, documentação do projeto, configurações ambientais locais de implantação e controle de PRs.
