...
| 3   | "Me ajuda a fazer o pyproject.toml no me projeto?"         | **Descartada** | A IA sugeriu colocar "setuptools.backends.legacy:build", o que acabou gerando o erro `Cannot import 'setuptools.backends.legacy'`. Além disse, a IA também não especificou quais pacotes eram para empacotar, o que gerou o erro `Multiple top-level packages discovered in a flat-layout: ['models', 'routes', 'database', 'services', 'migrations']`
